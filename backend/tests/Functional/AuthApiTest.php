...
class AuthApiTest extends ApiTestCase
{
    public function testRegisterCreatesUserAndConsentLog(): void
    {
        ...
        $user = $this->em->getRepository(User::class)->findOneBy(['email' => 'organizer@LocImmo.test']);
    ...
    {
        $this->jsonRequest('POST', '/api/auth/register', [
            'email' => 'participant@LocImmo.test',
            'password' => 'Password123!',
            'firstName' => 'Paul',
            'lastName' => 'Durand',
            'role' => 'participant',
        ...
        self::assertNull($this->em->getRepository(User::class)->findOneBy(['email' => 'participant@LocImmo.test']));
    }

    public function testLoginReturnsJwtToken(): void
    {
        $this->createUser([
            'email' => 'login@LocImmo.test',
    ...
    public function testAuthenticatedUserCanExportPersonalData(): void
    {
        ...
        self::assertSame('export@LocImmo.test', $data['personalData']['email']);
