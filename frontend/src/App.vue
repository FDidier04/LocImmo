  ...
  <div class="min-h-screen" style="background:var(--bg-base);color:var(--text-1)">
    ...
    <header class="sticky top-0 z-30 backdrop-blur-xl border-b"
      ...
      <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between gap-6">
        ...
        <router-link to="/" class="flex items-center gap-3 shrink-0">
          ...
          <span class="font-bold tracking-wide text-base hidden sm:block text-main"
            ...
            LOC<span class="text-orange-500">IMMO</span>
          </span>
        </router-link>
        ...
        <nav class="hidden md:flex items-center gap-6">
          ...
          <router-link class="nav-link" to="/events">Annonces</router-link>
          <router-link v-if="userStore.isAuthenticated" class="nav-link" to="/dashboard">Dashboard</router-link>
          <router-link v-if="userStore.isOrganizer" class="nav-link" to="/events/create">Publier</router-link>
      ...
      <Transition name="slide-up">
        <div v-if="mobileOpen" class="md:hidden border-t px-6 py-4 space-y-1"
          ...
          <router-link @click="mobileOpen=false" class="block py-2 text-sm text-sub hover:text-main transition-colors" to="/events">Annonces</router-link>
          ...
          <router-link v-if="userStore.isOrganizer" @click="mobileOpen=false" class="block py-2 text-sm text-sub hover:text-main transition-colors" to="/events/create">Publier une annonce</router-link>
    ...
    <section v-if="isDashboard" class="relative overflow-hidden">
      ...
      <div class="relative max-w-7xl mx-auto px-6 pt-24 pb-20">
        <div class="max-w-3xl">
          <div class="section-label mb-4">Offres immobilieres au Congo</div>

          <h1 class="text-5xl sm:text-6xl lg:text-7xl font-extrabold leading-[1.05] tracking-tight text-main">
            Trouver un bien,<br>
            <span class="gradient-text">simplement.</span>
          </h1>

          <p class="mt-6 text-lg text-sub max-w-xl leading-relaxed">
            LocImmo centralise les offres immobilieres a Brazzaville, Dolisie,
            Pointe-Noire et Nkayi avec une experience moderne et securisee.
          </p>
    ...
    <footer class="border-t" style="border-color:var(--border);background:var(--bg-card)">
      <div class="max-w-7xl mx-auto px-6 py-8 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div class="flex items-center gap-3">
          <div class="h-7 w-7 rounded-lg bg-orange-500 grid place-items-center font-black text-white text-xs">LI</div>
          <span class="text-sm text-muted">LocImmo — Symfony 6.4 + Vue 3</span>
        </div>
