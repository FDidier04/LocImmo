...
function consumeRegistrationFlash() {
  try {
    const message = sessionStorage.getItem('ImmoHub-registration-success')
    if (!message) return

    registrationSuccess.value = message
    sessionStorage.removeItem('ImmoHub-registration-success')
  } catch {
...
<template>
  <section class="space-y-8">
    ...
    <div class="grid gap-4 md:grid-cols-3">
      ...
      <article class="LI-card p-5">
        <div class="text-xs uppercase tracking-[0.2em] text-muted">Mes annonces</div>
    ...
    <div class="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
      <section class="glass p-6">
        ...
        <template v-if="nextRegistration">
          ...
          <router-link :to="`/events/${nextRegistration.event.id}`" class="btn-primary mt-5">Voir l annonce</router-link>
        </template>
        <template v-else>
          <h2 class="text-2xl font-extrabold text-main">Aucune inscription active</h2>
          <p class="mt-3 text-sm leading-6 text-sub">Explorez le catalogue public pour rejoindre un prochain annonce.</p>
          <router-link to="/events" class="btn-primary mt-5">Parcourir les annonces</router-link>
        </template>
      </section>
      ...
      <section class="glass p-6">
        ...
        <div class="grid gap-3">
          ...
          <router-link v-if="authStore.isOrganizer" to="/events/create" class="rounded-2xl border p-4 transition hover:-translate-y-0.5" style="border-color:var(--border); background:var(--bg-card)">
            <div class="font-bold text-main">Creer un annonce</div>
            <div class="mt-1 text-sm text-sub">Publier une nouvelle fiche annonce.</div>
          </router-link>
          <router-link v-if="authStore.isAdmin" to="/admin" class="rounded-2xl border p-4 transition hover:-translate-y-0.5" style="border-color:var(--border); background:var(--bg-card)">
            ...
            <div class="mt-1 text-sm text-sub">Piloter l ensemble des annonces de la Plateforme.</div>
          </router-link>
        </div>
      </section>
    </div>
    ...
    <section class="glass p-6">
      <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h2 class="text-2xl font-extrabold text-main">Mes annonces reserves</h2>
        </div>
