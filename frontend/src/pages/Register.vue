...
function setRegistrationFlash(message) {
  try {
    sessionStorage.setItem('ImmoHub-registration-success', message)
  } catch {
  ...
  <div class='min-h-[70vh] flex items-center justify-center py-12'>
    <div class='w-full max-w-lg'>
      <div class='text-center mb-10'>
        ...
        <p class='text-sub text-sm mt-2'>Rejoignez la Plateforme Immo Hub</p>
      </div>
      <div v-if='userStore.error' class='mb-6 p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm'>
        <template v-if='typeof userStore.error === "object"'>
          <pre class='whitespace-pre-wrap'>{{ JSON.stringify(userStore.error, null, 2) }}</pre>
        </template>
        <template v-else>
          {{ userStore.error }}
        </template>
      ...
      <div class='glass p-8'>
        <form @submit.prevent='submit' class='space-y-5'>
          <div class='grid grid-cols-2 gap-4'>
            <div>
              ...
              <input v-model='form.firstName' class='LI-input' placeholder='Prénom' />
            ...
            <div>
              ...
              <input v-model='form.lastName' class='LI-input' placeholder='Nom' />
          ...
          <div>
            ...
            <input v-model='form.email' type='email' class='LI-input' placeholder='email@exemple.com' />
          ...
          <div>
            ...
            <input v-model='form.password' type='password' class='LI-input' placeholder='Minimum 6 caractères' />
          ...
          <div>
            ...
            <input v-model='form.phone' class='LI-input' placeholder='+33 6 00 00 00 00' />
          </div>
          <div>
            <label class='block text-xs font-semibold text-muted uppercase tracking-wider mb-2'>Rôle</label>
            <select v-model='form.role' class='LI-input'>
          ...
          <div class='p-4 rounded-xl bg-orange-500/10 border border-orange-500/20'>
            <label class='flex items-start gap-3 cursor-pointer'>
              ...
              <span class='text-xs text-sub leading-relaxed'>
                J&apos;accepte que mes données personnelles soient traitées par Immo Hub conformément à la
        ...
        <div class='mt-6 pt-6 border-t LI-border text-center'>
