...
async function submitCreate() {
  ...
  try {
    ...
    successMessage.value = 'Nouvel annonce cree avec succes.'
    ...
    localError.value = payload?.message || (payload?.errors ? 'Veuillez corriger le formulaire.' : 'Impossible de creer cet annonce.')
  }
}
...
async function submitEdition() {
  ...
  try {
    ...
    successMessage.value = 'annonce mis a jour avec succes.'
    closeEditor()
  } catch (error) {
    const payload = error?.response?.data
    localError.value = payload?.message || (payload?.errors ? JSON.stringify(payload.errors) : 'Impossible de mettre a jour cet annonce.')
  }
}
...
async function togglePublish(event) {
  ...
  try {
    ...
    successMessage.value = wasPublished ? 'annonce depublie.' : 'annonce publie.'
  } catch (error) {
...
async function deleteEvent(event) {
  ...
  try {
    ...
    successMessage.value = 'annonce supprime.'
  } catch (error) {
    localError.value = error?.response?.data?.message || 'Impossible de supprimer cet annonce.'
  }
}
...
<template>
  <section class="space-y-8">
      ...
      <div class="relative grid gap-8 xl:grid-cols-[1.15fr_0.85fr] xl:items-end">
        <div class="space-y-5">
          ...
          <div class="flex flex-wrap items-center gap-3 text-xs">
            ...
            <span class="badge-green">Controle global des annonces</span>
          </div>
        </div>
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          <div class="rounded-3xl border p-5 text-white" style="background:rgba(255,255,255,0.07); border-color:rgba(255,255,255,0.1)">
            <div class="text-xs uppercase tracking-[0.22em] text-slate-300">annonces a venir</div>
            <div class="mt-3 text-4xl font-black">{{ metrics.upcoming }}</div>
            <p class="mt-2 text-sm text-slate-300">Sur {{ metrics.total }} annonces administres.</p>
          </div>
    ...
    <section class="glass p-6">
      <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        ...
        <div class="flex flex-wrap gap-3">
          <select v-model="contactStatusFilter" class="LI-input min-w-[180px]">
      ...
      <div v-if="contactLoading" class="mt-6 grid gap-4 lg:grid-cols-2">
        <div v-for="index in 2" :key="index" class="LI-card animate-pulse p-6">
      ...
      <div v-else class="mt-6 grid gap-4 lg:grid-cols-2">
        <article v-for="message in filteredContactMessages" :key="message.id" class="LI-card p-6">
    ...
    <section class="glass p-6">
      <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div class="max-w-2xl">
          ...
          <h2 class="text-2xl font-extrabold text-main">Creer un annonce depuis l'administration</h2>
          <p class="mt-3 text-sm leading-7 text-sub">
            L'administrateur peut publier directement un nouvel annonce, preparer un brouillon pour un organisateur
        ...
        <div class="flex flex-wrap gap-3">
          <button class="btn-primary" @click="createOpen = !createOpen">
            {{ createOpen ? 'Masquer le formulaire' : 'Creer un annonce' }}
          </button>
      ...
      <div v-if="createOpen" class="mt-6 grid gap-4 md:grid-cols-2">
        <div class="md:col-span-2">
          ...
          <input v-model="createForm.title" class="LI-input" placeholder="Ex: Sommet IA & Product 2026" />
        ...
        <div class="md:col-span-2">
          ...
          <textarea v-model="createForm.description" rows="4" class="LI-input resize-none"
        ...
        <div>
          ...
          <input v-model="createForm.eventDate" type="datetime-local" class="LI-input" />
        ...
        <div>
          ...
          <input v-model="createForm.endDate" type="datetime-local" class="LI-input" />
        ...
        <div>
          ...
          <input v-model="createForm.maxParticipants" type="number" min="1" class="LI-input" />
        ...
        <div class="md:col-span-2">
          ...
          <input v-model="createForm.location" class="LI-input" placeholder="Paris, Station F - Main Stage" />
        ...
        <label class="md:col-span-2 flex items-center gap-3 rounded-2xl border p-4 text-sm text-sub"
          ...
          Publier l'annonce des sa creation pour le rendre visible sur la Plateforme.
        </label>
        ...
        <div class="md:col-span-2 flex flex-wrap justify-end gap-3">
          ...
          <button class="btn-primary" @click="submitCreate()">Enregistrer l'annonce</button>
        </div>
      </div>
    </section>
      ...
      <section class="glass p-6">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h2 class="text-2xl font-extrabold text-main">Moderation des fiches annonce</h2>
          </div>

          <div class="flex flex-wrap gap-3">
            <button class="btn-ghost" @click="loadEvents()">Actualiser</button>
            <router-link to="/events/create" class="btn-primary">Creer un annonce</router-link>
          </div>
        </div>

        <div class="mt-6 grid gap-3 lg:grid-cols-2 xl:grid-cols-5">
          <input v-model="search" class="LI-input xl:col-span-2" placeholder="Titre, lieu, organisateur..." />

          <select v-model="statusFilter" class="LI-input">
            <option value="all">Tous les statuts</option>
          ...
          <select v-model="timelineFilter" class="LI-input">
            <option value="all">Toutes les dates</option>
            <option value="upcoming">A venir</option>
            <option value="past">Passes</option>
          </select>

          <select v-model="organizerFilter" class="LI-input">
        ...
        <div class="mt-3 grid gap-3 lg:grid-cols-[1fr_220px]">
          <div class="rounded-2xl border px-4 py-3 text-sm text-sub"
            style="background:var(--border-light); border-color:var(--border)">
            {{ filteredEvents.length }} resultat(s) affiches sur {{ metrics.total }} annonce(s). Les administrateurs peuvent publier,
            modifier et supprimer n'importe quelle fiche.
          </div>

          <select v-model="sortBy" class="LI-input">
      ...
      <aside class="glass p-6">
        ...
        <template v-if="spotlightEvent">
          ...
          <p class="mt-2 text-sm leading-6 text-sub">
            Cet annonce est actuellement la fiche publiee la plus proche de la saturation. C'est le bon candidat a surveiller
        ...
        <template v-else>
          ...
          <p class="mt-2 text-sm leading-6 text-sub">
            Publiez davantage d'annonces ou assouplissez vos filtres pour faire remonter une fiche a surveiller.
          </p>
        </template>
      </aside>
    </div>
    ...
    <div v-if="eventsStore.loading" class="grid gap-4 lg:grid-cols-2">
      <div v-for="index in 4" :key="index" class="LI-card animate-pulse p-6">
        <div class="h-5 w-32 rounded" style="background:var(--border)"></div>
        <div class="mt-4 h-4 w-2/3 rounded" style="background:var(--border-light)"></div>
        <div class="mt-2 h-4 w-full rounded" style="background:var(--border-light)"></div>
    ...
    <div v-else-if="filteredEvents.length === 0" class="glass p-12 text-center">
      ...
      <h2 class="mt-5 text-2xl font-extrabold text-main">Aucun annonce a afficher</h2>
      <p class="mx-auto mt-3 max-w-xl text-sm leading-6 text-sub">
        Ajustez les filtres, rechargez la liste ou creez un nouvel annonce pour alimenter le tableau d'administration.
      </p>
    ...
    <div v-else class="grid gap-5 lg:grid-cols-2">
      <article v-for="event in filteredEvents" :key="event.id" class="LI-card overflow-hidden">
    ...
    <section class="glass p-6">
      <div class="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        <div>
          ...
          <p class="mt-3 text-sm leading-7 text-sub">
            Le compte administrateur garde une vision transverse de la publication des annonces, mais la Plateforme reste
      ...
      <div class="w-full max-w-3xl rounded-[1.75rem] border p-6 md:p-8"
        ...
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 class="text-2xl font-extrabold text-main">Mettre a jour la fiche annonce</h2>
        ...
        <div class="mt-6 grid gap-4 md:grid-cols-2">
          <div class="md:col-span-2">
            <label class="mb-2 block text-xs font-semibold uppercase tracking-[0.18em] text-muted">Titre</label>
            <input v-model="editForm.title" class="LI-input" />
          </div>

          <div class="md:col-span-2">
            <label class="mb-2 block text-xs font-semibold uppercase tracking-[0.18em] text-muted">Description</label>
            <textarea v-model="editForm.description" rows="5" class="LI-input resize-none"></textarea>
          </div>

          <div>
            <label class="mb-2 block text-xs font-semibold uppercase tracking-[0.18em] text-muted">Debut</label>
            <input v-model="editForm.eventDate" type="datetime-local" class="LI-input" />
          </div>

          <div>
            <label class="mb-2 block text-xs font-semibold uppercase tracking-[0.18em] text-muted">Fin</label>
            <input v-model="editForm.endDate" type="datetime-local" class="LI-input" />
          </div>

          <div>
            <label class="mb-2 block text-xs font-semibold uppercase tracking-[0.18em] text-muted">Capacite</label>
            <input v-model="editForm.maxParticipants" type="number" min="1" class="LI-input" />
          </div>

          <div class="md:col-span-2">
            <label class="mb-2 block text-xs font-semibold uppercase tracking-[0.18em] text-muted">Lieu</label>
            <input v-model="editForm.location" class="LI-input" />
          </div>
        </div>
